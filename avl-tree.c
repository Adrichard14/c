...
struct AvlTree* newAvlTree(int key)
{
...
/* Given a non-empty binary search tree, return the
   ...
   searched. */
struct AvlTree * minValueAvlTree(struct AvlTree* node)
{
    struct AvlTree* current = node;
...
struct AvlTree* deleteAvlTree(struct AvlTree* root, int key)
{
    ...
    if ( key < root->key )
        root->left = deleteAvlTree(root->left, key);
 
    // If the key to be deleted is greater than the
    // root's key, then it lies in right subtree
    else if( key > root->key )
        root->right = deleteAvlTree(root->right, key);
    ...
    else
    {
        ...
        {
            struct AvlTree *temp = root->left ? root->left :
                                             root->right;
        ...
        else
        {
            ...
            struct AvlTree* temp = minValueAvlTree(root->right);
 
            // Copy the inorder successor's data to this node
            root->key = temp->key;
 
            // Delete the inorder successor
            root->right = deleteAvlTree(root->right, temp->key);
        }
    }
